    ...
    def compileSource(self, code: str) -> bytes:
        with tempfile.TemporaryDirectory() as tmpDir:
            ...
            result = subprocess.run(
                f"{self.exec} --target-env vulkan1.2 --amb --stdin --quiet -V -S comp  -o {path}",
...
class GLSLCCompiler(ShaderCompiler):
    ...
    def compileSource(self, code: str) -> bytes:
        result = subprocess.run(f"{self.exec} --target-env=vulkan1.2 -fshader-stage=comp -fauto-bind-uniforms -o - -",
    ...
    def compileFile(self, filepath: str) -> bytes:
        result = subprocess.run(f"{self.exec} --target-env=vulkan1.2 -fshader-stage=comp -fauto-bind-uniforms -o - {filepath}",
            shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
